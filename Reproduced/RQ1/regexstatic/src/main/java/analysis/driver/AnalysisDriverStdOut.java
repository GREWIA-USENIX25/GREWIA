...
public class AnalysisDriverStdOut {
    ...
    public static void performAnalysis(String  fileName, BufferedReader regexesReader, InterfaceSettings interfaceSettings, AnalysisSettings analysisSettings) {
                        ...
                        case EDA:
                            ...
                            if (isVerbose) {
                                ...
                                if (constructedEdaExploitString) {
                                    ...
                                    // 写入文件
                                    String _prefix = edaExploitString.getPrefix();
                                    String _pump = edaExploitString.getPumpByDegree(0);
                                    String text = getString(edaExploitString, _prefix, _pump);
                                    FileWriter fw = new FileWriter("AttackString/"+fileName);
                                    fw.write(text);
                                    fw.close();
                                } else {
                        ...
                        case IDA:
                            ...
                            if (isVerbose) {
                                ...
                                if (constructedIdaExploitString) {
                                    ...
                                    int infixByteSize = 0;
                                    int notInfixByteSize = 0;

                                    for (int i = 0; i < degree; i++) {
                                        if (i == 0) {
                                            System.out.println("\tPrefix:\t\t\"" + idaExploitString.getSeparatorByDegreeVisual(i) + "\"");
                                            notInfixByteSize += idaExploitString.getSeparatorByDegree(i).getBytes().length;
                                        } else {
                                        ...
                                        infixByteSize += idaExploitString.getPumpByDegree(i).getBytes().length;
                                    }
                                    System.out.println("\tSuffix:\t\t\"" + idaExploitString.getSuffixVisual() + "\"");

                                    // 写入文件
                                    String _suffix = idaExploitString.getSuffix();
                                    notInfixByteSize += _suffix.getBytes().length;
                                    // 根据prifix+pump*n+suffix构造一个大小为100kb的文本文件
                                    // 计算中缀需要重复的次数
                                    // int n = (100 * 1000 - notInfixByteSize) / infixByteSize;
                                    int n = (1024 * 1024 - notInfixByteSize) / infixByteSize;
                                    // 构造字符串
                                    StringBuilder sb = new StringBuilder();
                                    for (int i = 0; i < degree; i++) {
                                        if (i == 0) {
                                            sb.append(idaExploitString.getSeparatorByDegree(i));
                                        } else {
                                            sb.append(idaExploitString.getSeparatorByDegree(i));
                                        }
                                        for (int j = 0; j < n; j++) {
                                            sb.append(idaExploitString.getPumpByDegree(i));
                                        }
                                    }
                                    sb.append(idaExploitString.getSuffix());
    ...
    private static String getString(ExploitString edaExploitString, String _prifix, String _pump) {
        String _suffix = edaExploitString.getSuffix();
        // 根据prifix+pump*n+suffix构造一个大小为100kb的文本文件
        // 计算中缀需要重复的次数
        int n = (100 * 1000 - _prifix.length() - _suffix.length()) / _pump.length();
        // 构造中缀
        StringBuilder sb = new StringBuilder();
        for (int i = 0; i < n; i++) {
            sb.append(_pump);
