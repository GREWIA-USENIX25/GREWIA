...
public class Main {
	...
	public static void main(String[] args) {
		...
		for (String arg : args) {
			if (arg.startsWith("-")) {
				if (arg.contains("=")) {
					...
					if (settingName.equals(FILE_INPUT_SETTING)) {
						//只取路径最后的文件名
						fileName = settingValue.substring(settingValue.lastIndexOf("/") + 1);
					}
		...
		AnalysisSettings analysisSettings = new AnalysisSettings(nfaConstruction, 
						...
						timeout);		
		AnalysisDriverStdOut.performAnalysis(fileName, regexesReader, interfaceSettings, analysisSettings);
	}
