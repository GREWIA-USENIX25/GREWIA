...
parser.add_argument('--logdir', type=str, help='where to store output dir tarballs', default='logs')
...
fuzzer_bin_loc = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'fuzzer/build/fuzzer_stripped')
...
def do_work():
    ...
    while not stop:
        ...
        my_curr.execute("SELECT pattern, flags FROM regexps WHERE id = %s", (regexp_id,))
