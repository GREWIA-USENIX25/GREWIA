...
def main():
    ...
    if args.regex:
        ...
        with open(args.regex_file, 'r', encoding='utf8') as f:
            ascii_regex = f.read()
            bregex = ascii_regex.encode('utf8')
    else:
    ...
    async def do_fuzz():
        ...
        while True:
            ...
            command = ' '.join([fuzzer_binary + ' --bregexp ' + b64_regex + ' --lengths ' + str(current_length) + ' --widths ' + str(args.width) + ' --timeout ' + str(int(args.ftime / 1000) + 30) + ' --maxtot ' + str(maxtot) + ' ' + ' '.join(fuzzer_flags)])
            l.debug(f'Fuzzer command: {command}')
    ...
    l.debug(f'Pump report: {report}')

    # 如果report["class"]不等于UNKNOWN，说明有漏洞
    if report["class"] != 'UNKNOWN':
        # 如果report存在prefix、pump、suffix，按照prefix+pump*n+suffix,构造大小为100kb攻击字符串，并写入文件
        if 'prefix' in report and 'pump' in report and 'suffix' in report:
            # 将prefix、pump、suffix从bytes转为str
            prefix = report['prefix'].decode('utf-8')   
            _pump = report['pump'].decode('utf-8')
            suffix = report['suffix'].decode('utf-8')

            # 计算pump所占的字节数
            pumpByteSize = len(_pump.encode('utf-8'))

            # 计算pump的重复次数
            pumpRepeatTimes = 1024 * 1024 // pumpByteSize

            attackString = prefix + _pump * pumpRepeatTimes + suffix

            file_name = args.regex_file.split('/')[-1]
            # 将攻击字符串写入“AttackString/" + args.regex_file文件
            with open('AttackString/' + file_name, 'w') as f:
                f.write(attackString) 
        else:
            # 如果report不存在prefix、pump、suffix，将witness重复至至少100kb，并写入文件
            witness = bwitness.decode('utf-8')
            witnessByteSize = len(witness.encode('utf-8'))
            witnessRepeatTimes = 100 * 1024 // witnessByteSize
            attackString = witness * witnessRepeatTimes


            file_name = args.regex_file.split('/')[-1]
            # 将攻击字符串写入“AttackString/" + file_name文件
            with open('AttackString/' + file_name, 'w') as f:
                f.write(attackString)
