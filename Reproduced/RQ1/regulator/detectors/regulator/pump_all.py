...
parser.add_argument('--logdir', type=str, help='Path to pump logs for caching reruns', default='logs')
...
bin_inscount = 'fuzzer/build/fuzzer_stripped'
...
def do_work():
    ...
    while not stop:
        ...
        with global_lock:
            my_curr.execute("""
                ...
                WHERE rfr.length = 200
                    AND (rfr.max_observations > 3)
                    AND fuzzer_version >= 6
                    AND rfr.ts_pump_started IS NULL
            ...
            if l is None:
                ...
                id_, rfr_id = l
                curr.execute("UPDATE regexps_fuzz_results SET ts_pump_started = NOW()::timestamp where id = %s", (rfr_id,))
                db.commit()
        ...
        witness = None
        try:
            my_curr.execute("SELECT pattern, flags FROM regexps WHERE id = %s LIMIT 1", (id_,))
