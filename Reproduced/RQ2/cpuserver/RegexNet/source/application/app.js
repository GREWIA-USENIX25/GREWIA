...
app.use((req, res, next) => {
    ...
    //     const regex = /^(a*)*b$/;
    //     const isMatch = regex.test(req.headers['content']);
    //     console.log(`Regex match result: ${isMatch}`);
    // }
    ...
    //             resolve(regex.test(req.headers['content']));
    //         });
    //         try {
    //             isMatch = await Promise.race([regexPromise, timeoutPromise]);
    //         } catch (e) {
    //             process.exit(1);
    //         }
    //         console.log(`Regex match result: ${isMatch}`);
    ...
    if (req.headers['content']) {
        ...
        const regex = req.headers['regex'];
        const { execSync } = require('child_process');
        const command = `/usr/bin/time timeout 2s /home/cpuserver/RegexNet/build/node/bin/node /home/cpuserver/verify/JavaScript/benchmark.js '${regex}' ${path}`;
