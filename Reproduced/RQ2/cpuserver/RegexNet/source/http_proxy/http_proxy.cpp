...
public:
    ...
    void restart() {
        ...
        pid = fork();
        if (pid == 0){
            if(execle("/home/cpuserver/RegexNet/build/node/bin/node", "node", "/home/cpuserver/RegexNet/build/application/app.js", NULL, envp) < 0) {
...
public:
    ...
    int send_report(message_t* msg) {
        ...
        // } else {
        //     printf("send_report success: %d bytes\n", length);
        // }
    return length;
}
